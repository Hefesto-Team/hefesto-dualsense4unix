...
# uninstall.sh:917) e o `proton-pin.conf` é desfeito pelo `proton_pin.py
# --unlock` (uninstall.sh:1941), que não cita arquivo nenhum. O nome no disco
...
if [[ ! -d src/hefesto_dualsense4unix || ! -d scripts ]]; then
    ...
    #: ele (`doctor.sh:2545`), e pacote que leva o doctor sem ele fica com o
