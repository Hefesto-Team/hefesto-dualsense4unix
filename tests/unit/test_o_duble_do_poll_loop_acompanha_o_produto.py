...
produção** — de propósito, e é essa a virtude dele —, então toda chamada nova
precisa de um irmão no dublê. Ele já tinha cinco stubs exatamente por isso.
Faltou o sexto, e quem quebrou não era dono do arquivo que reprovou.
...
do dublê um irmão para cada chamada, nomeando o método que falta em vez de
deixar um `AttributeError` aparecer três arquivos adiante.
...
1. **`hasattr` passa por herança em método que o dublê não pretende ter.** A
   pergunta é *"o laço de produção chama isto?"*, e quem responde é o código do
...
O NÚMERO QUE DESENHA A RÉGUA — medido por AST, 20/09/2026
=========================================================
O `_poll_loop` chama **19** métodos de `self`. Nove deles rodam desconectado (a
condição do `while` mais os oito blocos do tique lento que subiram para antes do
gate); os outros dez só rodam com o controle na mesa. O dublê roda com
`is_connected()` devolvendo `False`, e por isso nunca precisou desses dez.

**Uma régua ingênua — a que exigisse os 19 — reprovaria NOVE vezes sem defeito
nenhum**, medido arrancando o corte desta régua em 20/09/2026. Por isso ela
reparte o laço no gate e exige só o que fica ANTES dele, que é exatamente o que
o dublê executa e onde a chamada nova caiu.
"""
...
def _indice_do_gate(laco: ast.While) -> int:
    ...
    for i, sentenca in enumerate(laco.body):
        if not isinstance(sentenca, ast.If):
            continue
        if not isinstance(sentenca.test, ast.UnaryOp) or not isinstance(
            sentenca.test.op, ast.Not
        ):
            continue
...
def _chamadas_de_self(nos: Iterable[ast.AST]) -> dict[str, int]:
    """`self.<nome>(...)` — só na primeira pessoa.

    `self.store.clear_controller_state()` fica de fora de propósito: quem tem de
    ter o método é o `store`, não o dublê.
    """
    ...
    for raiz in nos:
        for no in ast.walk(raiz):
            ...
            if (
                ...
                achadas.setdefault(alvo.attr, no.lineno)
    return achadas
...
def _reparte_no_gate(
    ...
    """Devolve (o que roda desconectado, o que só roda conectado).

    O primeiro conjunto é a preparação antes do `while`, a CONDIÇÃO do `while`
    (é ali que mora o `_is_stopping`), o corpo do laço até o gate, e o corpo do
    próprio gate — que o dublê executa, porque ele nasce desconectado.
    """
    laco = _o_while_do_laco(fn)
    corte = _indice_do_gate(laco)
    antes: list[ast.AST] = [
        *fn.body[: fn.body.index(laco)],
        laco.test,
        *laco.body[: corte + 1],
    ]
    depois: list[ast.AST] = [*laco.body[corte + 1 :], *laco.orelse]
    return _chamadas_de_self(antes), _chamadas_de_self(depois)
...
def _vocabulario(
    classe: ast.ClassDef, modulo: ast.Module, caminho: Path
) -> dict[str, int]:
...
def test_o_laco_de_producao_tem_um_gate_que_corta() -> None:
    ...
    """
    antes, depois = _reparte_no_gate(_producao())

    assert antes, "o gate de conexão é a primeira coisa do laço: nada roda desconectado?"
    assert depois, (
...
# ---------------------------------------------------------------------------
# 3. A régua: todo método chamado antes do gate tem irmão no dublê
# ---------------------------------------------------------------------------


def test_o_duble_tem_um_irmao_para_toda_chamada_antes_do_gate() -> None:
    """A entrega desta sprint, e a mensagem É a entrega.

    Quem lê esta reprovação é quem acabou de acrescentar a chamada ao
    `_poll_loop` — e ele não faz ideia de que existe um dublê. Então a mensagem
    diz o método, a linha do laço, o dublê, o arquivo dele, e a linha a escrever.

    Vale para as chamadas dentro de `with contextlib.suppress(Exception):`
    também, e ali vale ainda mais: lá um `AttributeError` não estoura, vira
    no-op silencioso — o dublê exercitaria um laço com um bloco a menos e
    ninguém saberia.

    O QUE A MORDIDA ARRANCA, e são duas:
    ...
    2. Devolva o stub e acrescente ao `_poll_loop`, antes do gate, um
       ...
       novo — que é o caso real que esta sprint cura.
    """
    antes, _ = _reparte_no_gate(_producao())
    faltando: list[str] = []

    for caminho, modulo, classe in _dubles_do_laco():
        tem = _vocabulario(classe, modulo, caminho)
        relativo = caminho.relative_to(RAIZ)
        for nome, linha in sorted(antes.items()):
            if nome in tem:
                continue
            faltando.append(
                f"  - `{nome}`  (chamado em daemon/lifecycle.py:{linha})\n"
                f"    falta em `{classe.name}`, {relativo}:{classe.lineno}\n"
                f"    escreva:  def {nome}(self, **_k: object) -> None: ..."
            )

    assert not faltando, (
        "O `_poll_loop` de produção chama, ANTES do gate de conexão, métodos que "
        "o dublê não tem. O dublê roda o laço de VERDADE — é essa a virtude "
        "dele —, então ele precisa de um irmão para cada chamada, senão a "
...
def test_a_regua_sabe_exatamente_onde_o_gate_corta() -> None:
    ...
    dentro do corpo do gate, a chamada ANINHADA depois do gate, e a chamada que
    não é em `self` (`self.store.x()` — quem precisa do método é o store).
    ...
    O QUE A MORDIDA ARRANCA: em `_reparte_no_gate`, troque
    `laco.body[: corte + 1]` por `laco.body`. A régua volta a ser a ingênua e
    este teste reprova nomeando `_depois_do_gate` e `_depois_aninhado` — e, no
    laço de verdade, a régua de cima passa a cobrar do dublê nove métodos que
    ele nunca executa (medido em 20/09/2026). Troque por `laco.body[:corte]` e
    este teste reprova por `_dentro_do_gate`, que o dublê EXECUTA porque nasce
    desconectado.
    """
    fn = _poll_loop_de(ast.parse(_LACO_DE_MENTIRA))
    antes, depois = _reparte_no_gate(fn)
    ...
    assert set(depois) == {"_depois_do_gate", "_depois_aninhado"}
    assert "nao_e_chamada_de_self" not in antes | depois


# ---------------------------------------------------------------------------
# 5. A recusa: um dublê que herda o produto não é mensurável por esta régua
# ---------------------------------------------------------------------------
