...
from hefesto.utils.xdg_paths import (
...
def test_ipc_socket_path_default_sem_env(
    ...
    """Sem env var definida, usa `hefesto.sock`."""
    monkeypatch.delenv(IPC_SOCKET_ENV_VAR, raising=False)
    path = ipc_socket_path()
    assert path.name == IPC_SOCKET_DEFAULT_NAME
    assert path == runtime_tmp / "hefesto.sock"


def test_ipc_socket_path_respeita_env_override(
    runtime_tmp: Path, monkeypatch: pytest.MonkeyPatch
):
    """Env var `HEFESTO_IPC_SOCKET_NAME` parametriza o nome do arquivo."""
    monkeypatch.setenv(IPC_SOCKET_ENV_VAR, "hefesto-smoke.sock")
    path = ipc_socket_path()
    assert path.name == "hefesto-smoke.sock"
    assert path == runtime_tmp / "hefesto-smoke.sock"
...
def test_ipc_socket_path_rejeita_nome_com_barra(
    ...
    monkeypatch.setenv(IPC_SOCKET_ENV_VAR, "../fora/hefesto.sock")
    path = ipc_socket_path()
    assert path.name == IPC_SOCKET_DEFAULT_NAME
