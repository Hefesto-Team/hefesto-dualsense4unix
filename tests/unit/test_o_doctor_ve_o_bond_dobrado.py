...
def _rodar(caminhos: list[str]) -> str:
    ...
    fim = subprocess.run(
        ...
        capture_output=True,
        text=True,
        env={"FALSA": falsa, "PATH": "/usr/bin:/bin"},
        check=False,
    )
    return fim.stdout
...
def test_o_recado_diz_o_gesto_e_nao_apaga_sozinho() -> None:
    ...
    assert "rm " not in corpo and "remove" not in corpo, (
        "o check APAGANDO bond: destruir pareamento é gesto dela — qual dos "
        "dois fica depende de onde ela quer o controle sentado."
    )
