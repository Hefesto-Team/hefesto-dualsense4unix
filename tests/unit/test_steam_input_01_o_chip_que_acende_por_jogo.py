...
def test_o_clique_escreve_a_vontade_e_constroi_a_ponte(lar) -> None:
    ...
    **fica em `"0"`**: esta linha reprova dizendo o valor que sobrou.
    ...
    assert aba.METODO_DA_RECARGA in p.chamadas if hasattr(aba, "METODO_DA_RECARGA") \
        else "launch_env.refresh" in p.chamadas, (
...
def test_o_gesto_nunca_fecha_a_steam_dela(lar, monkeypatch) -> None:
    ...
    A MORDIDA: devolva um `slo.with_steam_closed(...)` ao gesto e esta régua
    reprova nomeando a função — sem depender de um clique chegar até lá.
    ...
    fonte = "".join(
        ...
        for fn in (aba.modo_steam, aba._reconciliar_o_vdf, aba._os_que_ficam)
    )
    chamadas = {
...
def test_o_consentimento_que_fecha_a_steam_tem_UM_dono() -> None:  # noqa: N802
    ...
    **POR QUE ELA EXISTE:** o consentimento que fecha a Steam dela nasceu
    ...
    E ELA ALCANÇA O ESTADO, que é a metade que um import igual não garante: o
    relógio (`_ARMADO`) é UM só. Dois relógios sobre a MESMA Steam deixariam
    dois consentimentos pendurados ao mesmo tempo, e a segunda confirmação não
    teria como dizer a qual responde.
    """
    import importlib
