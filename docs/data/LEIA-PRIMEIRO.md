...
inteiro (<!--@caracteres-do-mapa-->1.419.441<!--/--> caracteres em células,
~<!--@tokens-do-mapa-->355<!--/--> mil tokens) ou não ler nada. O
veredito por lado das <!--@linhas-do-mapa-->312<!--/--> linhas, sem uma linha de prosa, custa 27.828
...
|---|---:|---|---|
| `docs/data/mapa-controles.csv` | <!--@bytes:docs/data/mapa-controles.csv-->1.479.225<!--/--> | **A FONTE.** <!--@linhas-do-mapa-->312<!--/--> linhas x <!--@colunas-do-mapa-->50<!--/--> colunas. Uma linha = uma feature em um controle. É portão, não documentação. | Sempre, mas **filtrado** — nunca com `Read` inteiro. Veja a seção 5. |
| `docs/data/ensaios.csv` | <!--@bytes:docs/data/ensaios.csv-->214.662<!--/--> | **O LASTRO.** <!--@linhas-do-caderno-->245<!--/--> ensaios x <!--@colunas-do-caderno-->14<!--/--> colunas. Cada linha é uma medição com hardware na mesa. Casa com o mapa por `linha_id == id`. **`degrau` e `ponte` existem no cabeçalho desde 20/08/2026 e quase ninguém as respondeu:** `ponte` está vazia em <!--@caderno-sem-ponte-->245<!--/--> ensaios e `degrau` em <!--@caderno-sem-degrau-->206<!--/-->. Não procure dado que ninguém escreveu ainda. | Quando a célula do mapa diz `medido` e você quer ver a medição. |
| `html/specs.html` | <!--@bytes:html/specs.html-->2.280.114<!--/--> | **DERIVADO** do CSV + do caderno, por `scripts/gerar-mapa.py`. Filtra no navegador. Mudou da raiz para `html/` em 25/08/2026. | **IA: não abra.** Ele embute o CSV inteiro como JSON: custa ~2x a fonte pela mesma informação. É excelente para olho humano com navegador, e péssimo para leitura por texto. |
| `docs/protocol/dualsense-referencia-canonica.md` | <!--@bytes:docs/protocol/dualsense-referencia-canonica.md-->118.100<!--/--> | **O PROTOCOLO.** O que o DualSense entende, byte a byte. | Quando a pergunta é "que report/offset/valor eu mando". Use a régua de conversão da seção 6. |
...
| `docs/process/METODO-DE-ISOLAMENTO.md` | <!--@bytes:docs/process/METODO-DE-ISOLAMENTO.md-->63.692<!--/--> | O ciclo de ensaio: perguntas de sanidade, oito passos, as armadilhas A-1..A-25. | Quando você vai **produzir** medição nova, não consumir. Cuidado: ele ainda ensina o nome de coluna `grau`, que o portão de hoje reprova (seção 6). |
| `scripts/check_paridade_transporte.py` | <!--@bytes:scripts/check_paridade_transporte.py-->145.521<!--/--> | **O PORTÃO** do mapa, e a melhor explicação de método da casa — a docstring nomeia cada regra e o defeito real que a fez nascer. | Antes de escrever no CSV. Leia a docstring inteira — ela vai da linha 2 à <!--@ultima-linha-da-docstring-do-portao-->343<!--/-->. |
| `scripts/eliminacao.py` | <!--@bytes:scripts/eliminacao.py-->11.675<!--/--> | **O JUIZ.** Lê o caderno e devolve um veredito por suspeito. | Seção 4. |
| `bancada.py` | <!--@bytes:bancada.py-->25.748<!--/--> | O formulário que grava no mapa. **A escada de degraus não nasce aqui:** ele a importa do portão (`from check_paridade_transporte import VALORES_DA_ESCADA`) desde 19/08/2026 — há um dono só. | Quando for editar célula. |
...
`id` é sempre exatamente `chave@controle` (confere em 308 de <!--@linhas-do-mapa-->312<!--/--> linhas), então
...
- `ponte_alcanca` / `ponte_de_onde_sei` — **o eixo de ponte** (20/08/2026), e
  ...
  <!--@linhas-do-mapa-->312<!--/--> linhas hoje, todas com `gamepad/dualsense` + `inferido-do-codigo`. Quem
...
cruzamento das 616 células (<!--@linhas-do-mapa-->312<!--/--> linhas x 2 lados), medido em 02/09/2026:
...
**O mapa da ignorância, de graça:** o caderno cobre **30 das <!--@linhas-do-mapa-->312<!--/--> linhas
...
o trecho abaixo em 22/08/2026)**:** só **37 das <!--@linhas-do-mapa-->312<!--/--> linhas** têm
...
- `provado_por` (mapa, <!--@mapa-com-provado-por-->83<!--/--> linhas:
  ...
  <!--@caderno-com-observado-por-->245<!--/--> linhas: `olho-dela`
