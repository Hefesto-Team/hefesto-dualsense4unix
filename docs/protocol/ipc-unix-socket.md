...
| `rumble.stop` | `daemon/ipc_handlers.py:5038` (`_handle_rumble_stop`) | Para o rumble e fixa (0, 0); no Modo Nativo SOLTA o par (NATIVO-RUMBLE-01). | **não** |
| `rumble.passthrough` | `daemon/ipc_handlers.py:5135` (`_handle_rumble_passthrough`) | Libera controle de rumble para jogo/UDP (BUG-RUMBLE-APPLY-IGNORED-01). | sim |
| `rumble.policy_set` | `daemon/ipc_handlers.py:5172` (`_handle_rumble_policy_set`) | Altera política global de intensidade de rumble (FEAT-RUMBLE-POLICY-01). | **não** |
| `rumble.policy_custom` | `daemon/ipc_handlers.py:5195` (`_handle_rumble_policy_custom`) | Define política "custom" com multiplicador explícito (FEAT-RUMBLE-POLICY-01). | **não** |
| `rumble.motores.set` | `daemon/ipc_handlers.py:5305` (`_handle_rumble_motores_set`) | `rumble.motores.set` — a barra de CADA motor, no perfil (VIBRACAO-POR-MOTOR-01). | sim |
| `sensor.set` | `daemon/ipc_handlers.py:5487` (`_handle_sensor_set`) | `sensor.set` — desliga giroscópio e acelerômetro DE VERDADE. | **não** |
...
| `daemon.reload` | `daemon/ipc_handlers.py:5694` (`_handle_daemon_reload`) | Aplica overrides parciais de config em runtime (REFACTOR-DAEMON-RELOAD-01). | sim |
| `launch_env.refresh` | `daemon/ipc_handlers.py:5738` (`_handle_launch_env_refresh`) | Rematerializa as envs de launch do wrapper (DEDUP-04) sob demanda. | **não** |
...
| `speaker.set` | `daemon/ipc_handlers.py:5760` (`_handle_speaker_set`) | `speaker.set` — volume/mudo/devolução do alto-falante (D4 + SOM-02). | sim |
| `mic.set` | `daemon/ipc_handlers.py:5912` (`_handle_mic_set`) | `mic.set` — mudo do microfone no FIRMWARE do controle (MIC-USB-01). | sim |
| `mic.canal.set` | `daemon/ipc_handlers.py:5985` (`_handle_mic_canal_set`) | `mic.canal.set` — O ATO INTEIRO do microfone (MICROFONE-UM-ATO-01). | **não** |
| `mic.led.set` | `daemon/ipc_handlers.py:6078` (`_handle_mic_led_set`) | `mic.led.set` — o LED do botão de mudo, e a DEVOLUÇÃO da posse dele. | sim |
| `mic.volume.set` | `daemon/ipc_handlers.py:6151` (`_handle_mic_volume_set`) | `mic.volume.set` — volume da CAPTURA no sistema (MIC-VOLUME-01). | sim |
| `mouse.emulation.set` | `daemon/ipc_handlers.py:6330` (`_handle_mouse_emulation_set`) | Liga/desliga emulação de mouse+teclado (FEAT-MOUSE-01). | sim |
| `mouse.emulation.restore` | `daemon/ipc_handlers.py:6405` (`_handle_mouse_emulation_restore`) | Restaura a emulação de mouse conforme a preferência persistida (HARM-06). | **não** |
| `keyboard.emulation.set` | `daemon/ipc_handlers.py:6426` (`_handle_keyboard_emulation_set`) | Liga/desliga a emulação de TECLADO (EMULACAO-NO-JOGO-01). | **não** |
| `gamepad.emulation.set` | `daemon/ipc_handlers.py:6535` (`_handle_gamepad_emulation_set`) | Liga/desliga o gamepad virtual e define a máscara (FEAT-DSX-GAMEPAD-FLAVOR-01). | **não** |
| `gamepad.mask.set` | `daemon/ipc_handlers.py:6462` (`_handle_gamepad_mask_set`) | A máscara de UM aparelho: `gamepad.mask.set {uniq, flavor}`. | **não** |
| `coop.set` | `daemon/ipc_handlers.py:6640` (`_handle_coop_set`) | Liga o co-op local; RECUSA desligar (FEAT-DSX-COOP-LOCAL-01). | sim |
| `coop.sync` | `daemon/ipc_handlers.py:6694` (`_handle_coop_sync`) | Roda UM ciclo cheio de reconciliação do co-op (`sync(force=True)`). | sim |
| `daemon.emulation.suppress` | `daemon/ipc_handlers.py:6731` (`_handle_emulation_suppress`) | Liga/desliga o modo jogo (suprime emulação mouse/teclado). | sim |
...
| `machine.declare` | `daemon/ipc_handlers.py:6748` (`_handle_machine_declare`) | Grava no `maquina.json` o que ela DECLAROU sobre a mesa (CONFIG-03). | sim |
| `plugin.list` | `daemon/ipc_handlers.py:6853` (`_handle_plugin_list`) | Lista plugins carregados no daemon (FEAT-PLUGIN-01). | **não** |
| `plugin.reload` | `daemon/ipc_handlers.py:6865` (`_handle_plugin_reload`) | Recarrega plugins do disco (FEAT-PLUGIN-01). | **não** |
