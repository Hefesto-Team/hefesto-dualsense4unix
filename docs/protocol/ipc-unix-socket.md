...
|---|---|---|---|
| `profile.switch` | `daemon/ipc_handlers.py:882` (`_handle_profile_switch`) | Aplica perfil escolhido pelo usuário (entrada manual via IPC). | sim |
| `profile.list` | `daemon/ipc_handlers.py:1001` (`_handle_profile_list`) | _(o handler não tem docstring)_ | sim |
| `profile.apply_draft` | `daemon/ipc_handlers.py:1018` (`_handle_profile_apply_draft`) | Aplica draft completo em ordem canonica: leds -> triggers -> rumble -> mouse. | sim |
| `trigger.set` | `daemon/ipc_handlers.py:1337` (`_handle_trigger_set`) | _(o handler não tem docstring)_ | sim |
| `trigger.reset` | `daemon/ipc_handlers.py:1384` (`_handle_trigger_reset`) | Devolve o gatilho ao perfil e LIBERA a trava manual dele (R-19). | sim |
| `led.set` | `daemon/ipc_handlers.py:1438` (`_handle_led_set`) | _(o handler não tem docstring)_ | sim |
| `led.auto_release` | `daemon/ipc_handlers.py:1630` (`_handle_led_auto_release`) | Devolve a luz ao automático: solta a trava manual de `"led"` — e SÓ ela. | sim |
| `rumble.set` | `daemon/ipc_handlers.py:5228` (`_handle_rumble_set`) | Fixa a vibração; RECUSA no Modo Nativo (FEAT-RUMBLE-POLICY-01 + NATIVO-RUMBLE-01). | **não** |
| `rumble.stop` | `daemon/ipc_handlers.py:5338` (`_handle_rumble_stop`) | Para o rumble e fixa (0, 0); no Modo Nativo SOLTA o par (NATIVO-RUMBLE-01). | **não** |
| `rumble.passthrough` | `daemon/ipc_handlers.py:5440` (`_handle_rumble_passthrough`) | Libera controle de rumble para jogo/UDP (BUG-RUMBLE-APPLY-IGNORED-01). | sim |
| `rumble.policy_set` | `daemon/ipc_handlers.py:5477` (`_handle_rumble_policy_set`) | Altera política global de intensidade de rumble (FEAT-RUMBLE-POLICY-01). | **não** |
| `rumble.policy_custom` | `daemon/ipc_handlers.py:5500` (`_handle_rumble_policy_custom`) | Define política "custom" com multiplicador explícito (FEAT-RUMBLE-POLICY-01). | **não** |
| `rumble.motores.set` | `daemon/ipc_handlers.py:5610` (`_handle_rumble_motores_set`) | `rumble.motores.set` — a barra de CADA motor, no perfil (VIBRACAO-POR-MOTOR-01). | sim |
| `sensor.set` | `daemon/ipc_handlers.py:5792` (`_handle_sensor_set`) | `sensor.set` — desliga giroscópio e acelerômetro DE VERDADE. | **não** |
| `daemon.status` | `daemon/ipc_handlers.py:2419` (`_handle_daemon_status`) | _(o handler não tem docstring)_ | sim |
| `daemon.state_full` | `daemon/ipc_handlers.py:2804` (`_handle_daemon_state_full`) | Estado completo pra GUI consumir a 20Hz. | sim |
| `daemon.pause` | `daemon/ipc_handlers.py:2755` (`_handle_daemon_pause`) | Pausa o despacho de input sem matar o daemon (FEAT-DAEMON-PAUSE-RESUME-01). | **não** |
| `daemon.resume` | `daemon/ipc_handlers.py:2760` (`_handle_daemon_resume`) | Retoma o despacho de input (FEAT-DAEMON-PAUSE-RESUME-01). | **não** |
| `autoswitch.lock` | `daemon/ipc_handlers.py:2765` (`_handle_autoswitch_lock`) | Congela/descongela a troca AUTOMÁTICA de perfil (FEAT-AUTOSWITCH-LOCK-01). | **não** |
| `native.mode.set` | `daemon/ipc_handlers.py:2785` (`_handle_native_mode_set`) | Liga/desliga o Modo Nativo — "release total" do controle (FEAT-NATIVE-MODE-01). | sim |
| `controller.list` | `daemon/ipc_handlers.py:4958` (`_handle_controller_list`) | Lista os controles do daemon; opt-in `external` soma o inventário 8BIT-01. | sim |
| `controller.target.set` | `daemon/ipc_handlers.py:5113` (`_handle_controller_target_set`) | Define o ALVO das ações de output (FEAT-DSX-CONTROLLER-SELECTOR-01). | **não** |
| `daemon.reload` | `daemon/ipc_handlers.py:5999` (`_handle_daemon_reload`) | Aplica overrides parciais de config em runtime (REFACTOR-DAEMON-RELOAD-01). | sim |
| `launch_env.refresh` | `daemon/ipc_handlers.py:6043` (`_handle_launch_env_refresh`) | Rematerializa as envs de launch do wrapper (DEDUP-04) sob demanda. | **não** |
| `lightbar.reset` | `daemon/ipc_handlers.py:5179` (`_handle_lightbar_reset`) | Manda o Reset LED state (0x08) sob demanda — INSTRUMENTO de medição. | sim |
| `debug.player_leds` | `daemon/ipc_handlers.py:5207` (`_handle_debug_player_leds`) | Liga/desliga a escrita do LED de JOGADOR — INSTRUMENTO de eliminação. | **não** |
| `speaker.set` | `daemon/ipc_handlers.py:6065` (`_handle_speaker_set`) | `speaker.set` — volume/mudo/devolução do alto-falante (D4 + SOM-02). | sim |
| `mic.set` | `daemon/ipc_handlers.py:6319` (`_handle_mic_set`) | `mic.set` — mudo do microfone no FIRMWARE do controle (MIC-USB-01). | sim |
| `mic.canal.set` | `daemon/ipc_handlers.py:6392` (`_handle_mic_canal_set`) | `mic.canal.set` — O ATO INTEIRO do microfone (MICROFONE-UM-ATO-01). | **não** |
| `mic.led.set` | `daemon/ipc_handlers.py:6535` (`_handle_mic_led_set`) | `mic.led.set` — o LED do botão de mudo, e a DEVOLUÇÃO da posse dele. | sim |
| `mic.volume.set` | `daemon/ipc_handlers.py:6608` (`_handle_mic_volume_set`) | `mic.volume.set` — volume da CAPTURA no sistema (MIC-VOLUME-01). | sim |
| `mouse.emulation.set` | `daemon/ipc_handlers.py:6794` (`_handle_mouse_emulation_set`) | Liga/desliga emulação de mouse+teclado (FEAT-MOUSE-01). | sim |
| `mouse.emulation.restore` | `daemon/ipc_handlers.py:6869` (`_handle_mouse_emulation_restore`) | Restaura a emulação de mouse conforme a preferência persistida (HARM-06). | **não** |
| `desktop.arranjo.apply` | `daemon/ipc_handlers.py:6890` (`_handle_desktop_arranjo_apply`) | Carrega no aparelho o que a aba Navegação gravou no perfil ATIVO. | **não** |
| `keyboard.emulation.set` | `daemon/ipc_handlers.py:6928` (`_handle_keyboard_emulation_set`) | Liga/desliga a emulação de TECLADO (EMULACAO-NO-JOGO-01). | **não** |
| `gamepad.emulation.set` | `daemon/ipc_handlers.py:7037` (`_handle_gamepad_emulation_set`) | Liga/desliga o gamepad virtual e define a máscara (FEAT-DSX-GAMEPAD-FLAVOR-01). | **não** |
| `gamepad.mask.set` | `daemon/ipc_handlers.py:6964` (`_handle_gamepad_mask_set`) | A máscara de UM aparelho: `gamepad.mask.set {uniq, flavor}`. | **não** |
| `coop.set` | `daemon/ipc_handlers.py:7142` (`_handle_coop_set`) | Liga o co-op local; RECUSA desligar (FEAT-DSX-COOP-LOCAL-01). | sim |
| `coop.sync` | `daemon/ipc_handlers.py:7196` (`_handle_coop_sync`) | Roda UM ciclo cheio de reconciliação do co-op (`sync(force=True)`). | sim |
| `daemon.emulation.suppress` | `daemon/ipc_handlers.py:7233` (`_handle_emulation_suppress`) | Liga/desliga o modo jogo (suprime emulação mouse/teclado). | sim |
...
| `identity.renumber` | `daemon/ipc_handlers.py:1672` (`_handle_identity_renumber`) | Reordena a FILA de preferência (DualSense + externos) — ONDA-U/NUM-01. | sim |
| `identity.number.set` | `daemon/ipc_handlers.py:1986` (`_handle_identity_number_set`) | Atribui o NÚMERO EXIBIDO de UM controle (PLAYER-01, 25/07). | sim |
| `machine.declare` | `daemon/ipc_handlers.py:7250` (`_handle_machine_declare`) | Grava no `maquina.json` o que ela DECLAROU sobre a mesa (CONFIG-03). | sim |
| `plugin.list` | `daemon/ipc_handlers.py:7355` (`_handle_plugin_list`) | Lista plugins carregados no daemon (FEAT-PLUGIN-01). | **não** |
| `plugin.reload` | `daemon/ipc_handlers.py:7367` (`_handle_plugin_reload`) | Recarrega plugins do disco (FEAT-PLUGIN-01). | **não** |
| `radio.ponte.ligar_aqui` | `daemon/ipc_handlers.py:7622` (`_handle_radio_ponte_ligar_aqui`) | «Ligar aqui»: a ponte deste controle sobe além do limite do adaptador. | **não** |
| `radio.mover` | `daemon/ipc_handlers.py:7664` (`_handle_radio_mover`) | «Mover» um aparelho para um adaptador, ou o «Conectar» (D8). | sim |
| `mira.set` | `daemon/ipc_handlers.py:7751` (`_handle_mira_set`) | `mira.set` — o chip «Mira Virtual» e os ajustes da Calibrar, POR CONTROLE. | sim |
...
`led`, e a trava mora no `StateStore` do daemon, sem dono por controle. O campo
`escopo` diz isso na resposta em vez de deixar quem chama supor.

O DEFEITO QUE ELE FECHA, medido em 29/08/2026: `led.set` e `led.player_set`
ARMAVAM a trava e nenhuma linha de `src/` a soltava. Enquanto qualquer categoria
está armada o autoswitch não reaplica o perfil por mudança de janela, então a
única saída era trocar de perfil na mão — um gesto que a pessoa não tem como
saber que precisa fazer. `trigger` tinha o `trigger.reset`, `rumble` tinha o
`rumble.passthrough`; a luz não tinha porta de volta.

QUEM CHAMA é o botão "Automático" da aba Iluminação, e ele é a TERCEIRA de três
chamadas, nesta ordem: `lightbar.reset` (larga o claim da barra), `led.set`
(pinta a cor do slot, para a barra não ficar preta) e então `led.auto_release`.
A ordem é o conserto — o `led.set` do meio ARMA a categoria, então um release
...
| Método               | Parâmetros                                         | Retorno                                                        |
|----------------------|----------------------------------------------------|----------------------------------------------------------------|
| `rumble.motores.set` | `{uniq?: str, forte_pct?: 0-100, fraco_pct?: 0-100}` | `{status, uniq, perfil, gravado, forte_pct, fraco_pct}` |
...
Por isso o valor vai para o **perfil**, por peça
(`ControllerRumbleOverride.motor_forte_pct` / `motor_fraco_pct`), e não para o
`DaemonConfig`: `rumble.set {weak, strong}` continua sendo o comando de tremer
AGORA, e este método não o chama — camadas diferentes, pela mesma razão que
separa `mic.set` de `mic.volume.set`.

**Campo omitido não mexe naquela barra**; `100` nos dois **apaga** os campos (no
aparelho "escreveu 100" e "não escreveu" são o mesmo fator 1,0, e a chave a
menos é o que mantém o downgrade possível). A seção `rumble` da peça só cai
inteira quando fica vazia — o degrau daquela peça (`policy`, o teto do card do
cabo) mora ali e não é deste gesto. Nada mudou = `gravado: false`, sem regravar:
um `save` troca a data do arquivo e faz o daemon reaplicar o perfil.

`status` pode ser `"ok"`, `"sem_controle"` (mesa vazia), `"sem_endereco"` (o
alvo não é um MAC de peça de plástico) ou `"sem_perfil"`; os três últimos vêm
com `motivo`. A faixa 0-100 é recusada na BORDA do esquema, com a frase que
EXPLICA por que a barra não passa de 100 — ela é o segundo fator, e quem
amplifica é o degrau.

**A leitura de volta é o `state_full`**: `rumble_motores` traz `{uniq:
{forte_pct, fraco_pct}}` **só de quem tem opinião**, e `rumble_motor_pct_padrao`
traz o 100 do "sem opinião" para a tela não digitá-lo. A fonte é a MESMA que
`apply_game_rumble` multiplica (`gamepad._motores_do_perfil_ativo`), e não uma
