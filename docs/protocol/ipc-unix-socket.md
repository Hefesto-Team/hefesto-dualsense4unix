...
| `rumble.set` | `daemon/ipc_handlers.py:5109` (`_handle_rumble_set`) | Fixa a vibração; RECUSA no Modo Nativo (FEAT-RUMBLE-POLICY-01 + NATIVO-RUMBLE-01). | **não** |
| `rumble.stop` | `daemon/ipc_handlers.py:5219` (`_handle_rumble_stop`) | Para o rumble e fixa (0, 0); no Modo Nativo SOLTA o par (NATIVO-RUMBLE-01). | **não** |
| `rumble.passthrough` | `daemon/ipc_handlers.py:5321` (`_handle_rumble_passthrough`) | Libera controle de rumble para jogo/UDP (BUG-RUMBLE-APPLY-IGNORED-01). | sim |
| `rumble.policy_set` | `daemon/ipc_handlers.py:5358` (`_handle_rumble_policy_set`) | Altera política global de intensidade de rumble (FEAT-RUMBLE-POLICY-01). | **não** |
| `rumble.policy_custom` | `daemon/ipc_handlers.py:5381` (`_handle_rumble_policy_custom`) | Define política "custom" com multiplicador explícito (FEAT-RUMBLE-POLICY-01). | **não** |
| `rumble.motores.set` | `daemon/ipc_handlers.py:5491` (`_handle_rumble_motores_set`) | `rumble.motores.set` — a barra de CADA motor, no perfil (VIBRACAO-POR-MOTOR-01). | sim |
| `sensor.set` | `daemon/ipc_handlers.py:5673` (`_handle_sensor_set`) | `sensor.set` — desliga giroscópio e acelerômetro DE VERDADE. | **não** |
...
| `controller.list` | `daemon/ipc_handlers.py:4839` (`_handle_controller_list`) | Lista os controles do daemon; opt-in `external` soma o inventário 8BIT-01. | sim |
| `controller.target.set` | `daemon/ipc_handlers.py:4994` (`_handle_controller_target_set`) | Define o ALVO das ações de output (FEAT-DSX-CONTROLLER-SELECTOR-01). | **não** |
| `daemon.reload` | `daemon/ipc_handlers.py:5880` (`_handle_daemon_reload`) | Aplica overrides parciais de config em runtime (REFACTOR-DAEMON-RELOAD-01). | sim |
| `launch_env.refresh` | `daemon/ipc_handlers.py:5924` (`_handle_launch_env_refresh`) | Rematerializa as envs de launch do wrapper (DEDUP-04) sob demanda. | **não** |
| `lightbar.reset` | `daemon/ipc_handlers.py:5060` (`_handle_lightbar_reset`) | Manda o Reset LED state (0x08) sob demanda — INSTRUMENTO de medição. | sim |
| `debug.player_leds` | `daemon/ipc_handlers.py:5088` (`_handle_debug_player_leds`) | Liga/desliga a escrita do LED de JOGADOR — INSTRUMENTO de eliminação. | **não** |
| `speaker.set` | `daemon/ipc_handlers.py:5946` (`_handle_speaker_set`) | `speaker.set` — volume/mudo/devolução do alto-falante (D4 + SOM-02). | sim |
| `mic.set` | `daemon/ipc_handlers.py:6099` (`_handle_mic_set`) | `mic.set` — mudo do microfone no FIRMWARE do controle (MIC-USB-01). | sim |
| `mic.canal.set` | `daemon/ipc_handlers.py:6172` (`_handle_mic_canal_set`) | `mic.canal.set` — O ATO INTEIRO do microfone (MICROFONE-UM-ATO-01). | **não** |
| `mic.led.set` | `daemon/ipc_handlers.py:6315` (`_handle_mic_led_set`) | `mic.led.set` — o LED do botão de mudo, e a DEVOLUÇÃO da posse dele. | sim |
| `mic.volume.set` | `daemon/ipc_handlers.py:6388` (`_handle_mic_volume_set`) | `mic.volume.set` — volume da CAPTURA no sistema (MIC-VOLUME-01). | sim |
| `mouse.emulation.set` | `daemon/ipc_handlers.py:6574` (`_handle_mouse_emulation_set`) | Liga/desliga emulação de mouse+teclado (FEAT-MOUSE-01). | sim |
| `mouse.emulation.restore` | `daemon/ipc_handlers.py:6649` (`_handle_mouse_emulation_restore`) | Restaura a emulação de mouse conforme a preferência persistida (HARM-06). | **não** |
| `keyboard.emulation.set` | `daemon/ipc_handlers.py:6670` (`_handle_keyboard_emulation_set`) | Liga/desliga a emulação de TECLADO (EMULACAO-NO-JOGO-01). | **não** |
| `gamepad.emulation.set` | `daemon/ipc_handlers.py:6779` (`_handle_gamepad_emulation_set`) | Liga/desliga o gamepad virtual e define a máscara (FEAT-DSX-GAMEPAD-FLAVOR-01). | **não** |
| `gamepad.mask.set` | `daemon/ipc_handlers.py:6706` (`_handle_gamepad_mask_set`) | A máscara de UM aparelho: `gamepad.mask.set {uniq, flavor}`. | **não** |
| `coop.set` | `daemon/ipc_handlers.py:6884` (`_handle_coop_set`) | Liga o co-op local; RECUSA desligar (FEAT-DSX-COOP-LOCAL-01). | sim |
| `coop.sync` | `daemon/ipc_handlers.py:6938` (`_handle_coop_sync`) | Roda UM ciclo cheio de reconciliação do co-op (`sync(force=True)`). | sim |
| `daemon.emulation.suppress` | `daemon/ipc_handlers.py:6975` (`_handle_emulation_suppress`) | Liga/desliga o modo jogo (suprime emulação mouse/teclado). | sim |
...
| `machine.declare` | `daemon/ipc_handlers.py:6992` (`_handle_machine_declare`) | Grava no `maquina.json` o que ela DECLAROU sobre a mesa (CONFIG-03). | sim |
| `plugin.list` | `daemon/ipc_handlers.py:7097` (`_handle_plugin_list`) | Lista plugins carregados no daemon (FEAT-PLUGIN-01). | **não** |
| `plugin.reload` | `daemon/ipc_handlers.py:7109` (`_handle_plugin_reload`) | Recarrega plugins do disco (FEAT-PLUGIN-01). | **não** |
