...
def mic_existe(ctx: Contexto, o: dict[str, Any], p: Any) -> None:
    ...
    *"Desligado, nenhum programa o enxerga"* — e isso é a PONTE, que existe ou
    ...
    (`ipc_handlers.py:6810`, QUATRO-MICROFONES-01): *"o 'Aplicar' tem de VALER
...
def _com_o_teto(prof: Any, chave: str, policy: str | None) -> Any:
    ...
    `cfg.rumble is None` (`profiles/manager.py:2372`) e `"policy" not in
    model_fields_set` (`:2375`). O primeiro é o que o esquema chama de "campo
    ...
    1,0 (`profiles/manager.py:2392-2393`) — guardar o override só deixaria no
...
# `_handle_machine_declare` (`daemon/ipc_handlers.py:6810`) → `maquina.json`, e
