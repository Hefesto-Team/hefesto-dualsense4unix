...
`_recusou_dizendo` (`hefesto_vivo.py:3663`), e o `except` de `trabalhar()` põe a
...
* das TRÊS opções que a tela dela oferece, DUAS viraram clique morto — e uma
  ...
  por contrato**: `_recusou_dizendo` (`hefesto_vivo.py:3663`) levava à tela a
...
#: (`_jogo_no_controle_do_desktop`, `daemon/lifecycle.py:2917`, e o
...
# fábrica (`profiles/manager.py:681`, `core/acoes_de_botao.resolver`, que nunca
...
def atalhos_que_param_de_valer(p: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    **É A METADE VISÍVEL DO DEFEITO §3-1**, e o defeito é do produto, não desta
    aba: `apply_button_actions` (`profiles/manager.py:681`) roda DEPOIS do
...
def _recusa_do_teclado(resposta: Any) -> str:
    ...
    *"para a janela não precisar de uma segunda chamada só para saber se o
    device subiu"* (`daemon/ipc_handlers.py:6670`). Quem o traduz é
...
def teclado(ctx: Contexto, o: dict[str, Any], p: Any) -> dict[str, Any] | None:
    ...
    *"'Só dentro do jogo' já existe, e é o `suppress_desktop_emulation`"*. **Os
    ...
    2. `apply_profile_suppression` (`daemon/lifecycle.py:2566`) recebe esse
    ...
    `set_mouse_emulation` (`daemon/lifecycle.py:1739`).

    Do outro lado, o teclado não mexe no gamepad virtual em momento nenhum.
    Quem o liga e desliga é o
    `set_keyboard_emulation` (`daemon/lifecycle.py:2075`): ele cria ou destrói o
...
def tecla_escrita(ctx: Contexto, o: dict[str, Any], p: Any) -> dict[str, Any] | None:
    ...
    `_recusou_dizendo` do piloto (`hefesto_vivo.py:3663`) guarda a classe da
...
#:    (`daemon/lifecycle.py:1805`) chama `set_mouse_emulation(pref, …)` e, com a
