...
#: (`_jogo_no_controle_do_desktop`, `daemon/lifecycle.py:2480`, e o
...
def teclado(ctx: Contexto, o: dict[str, Any], p: Any) -> dict[str, Any] | None:
    ...
    *"'Só dentro do jogo' já existe, e é o `suppress_desktop_emulation`"*. **Os
    ...
    2. `apply_profile_suppression` (`daemon/lifecycle.py:2161`) recebe esse
    ...
    `set_mouse_emulation` (`daemon/lifecycle.py:1537`).

    Do outro lado, o teclado não mexe no gamepad virtual em momento nenhum.
    Quem o liga e desliga é o
    `set_keyboard_emulation` (`daemon/lifecycle.py:1670`): ele cria ou destrói o
...
#:    (`daemon/lifecycle.py:1603`) chama `set_mouse_emulation(pref, …)` e, com a
