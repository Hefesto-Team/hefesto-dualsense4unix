...
#: (`_jogo_no_controle_do_desktop`, `daemon/lifecycle.py:2392`, e o
...
def _recusa_do_teclado(resposta: Any) -> str:
    ...
    *"para a janela não precisar de uma segunda chamada só para saber se o
    device subiu"* (`daemon/ipc_handlers.py:6450`). Quem o traduz é
...
def teclado(ctx: Contexto, o: dict[str, Any], p: Any) -> dict[str, Any] | None:
    ...
    *"'Só dentro do jogo' já existe, e é o `suppress_desktop_emulation`"*. **Os
    ...
    2. `apply_profile_suppression` (`daemon/lifecycle.py:2073`) recebe esse
    ...
    `set_mouse_emulation` (`daemon/lifecycle.py:1449`).

    Do outro lado, o teclado não mexe no gamepad virtual em momento nenhum.
    Quem o liga e desliga é o
    `set_keyboard_emulation` (`daemon/lifecycle.py:1582`): ele cria ou destrói o
...
#:    (`daemon/lifecycle.py:1515`) chama `set_mouse_emulation(pref, …)` e, com a
