...
`_recusou_dizendo` (`hefesto_vivo.py:3371`), e o `except` de `trabalhar()` põe a
...
* das TRÊS opções que a tela dela oferece, DUAS viraram clique morto — e uma
  ...
  por contrato**: `_recusou_dizendo` (`hefesto_vivo.py:3371`) levava à tela a
...
#: (`_jogo_no_controle_do_desktop`, `daemon/lifecycle.py:2433`, e o
...
def _o_que_a_pagina_oferece() -> frozenset[str]:
    ...
    recarrega (`hefesto_vivo.py:4605`, `_ir`), e reabrir também. Ler o DOM em vez do
...
# fábrica (`profiles/manager.py:645`, `core/acoes_de_botao.resolver`, que nunca
...
def atalhos_que_param_de_valer(p: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    **É A METADE VISÍVEL DO DEFEITO §3-1**, e o defeito é do produto, não desta
    aba: `apply_button_actions` (`profiles/manager.py:645`) roda DEPOIS do
...
#: (`daemon/ipc_handlers.py:154`) lê a AUSÊNCIA como `"profile"`, e a assimetria é
...
def modo(ctx: Contexto, o: dict[str, Any], p: Any) -> dict[str, Any] | None:
    ...
    gamepad virtual (`daemon/lifecycle.py:1490` — ligar o mouse PARA o vpad). Se
...
def teclado(ctx: Contexto, o: dict[str, Any], p: Any) -> dict[str, Any] | None:
    ...
    *"'Só dentro do jogo' já existe, e é o `suppress_desktop_emulation`"*. **Os
    ...
    2. `apply_profile_suppression` (`daemon/lifecycle.py:2114`) recebe esse
...
def tecla_escrita(ctx: Contexto, o: dict[str, Any], p: Any) -> dict[str, Any] | None:
    ...
    `_recusou_dizendo` do piloto (`hefesto_vivo.py:3371`) guarda a classe da
...
#:    (`daemon/lifecycle.py:1556`) chama `set_mouse_emulation(pref, …)` e, com a
...
SEM_GESTO = {
    ...
    "acao-do-gesto": "os cinco combos são callbacks montados em código "
                     "(`daemon/subsystems/hotkey.py:90,418`), não dado. O "
