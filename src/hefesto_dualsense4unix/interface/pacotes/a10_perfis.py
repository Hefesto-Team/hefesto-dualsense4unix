...
def _mesa_com_rotulo(mesa: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    `_linhas_da_guarda` lê `controle.get("rotulo")` (`perfis_web.py:609`).
    ...
    **Ele fazia o CONTRÁRIO**, e a medição é uma foto: aquele visor punha em
    ...
    `_linhas_da_guarda` lê `controle.get("plastico")` (`perfis_web.py:608`) e
...
def _rotulo_do_remover(alvo: str) -> str:
    ...
    **FATO CADUCO, SUBSTITUÍDO — 02/09/2026.** Aqui estava escrito que *"a
    ...
    `_recusou_dizendo` (`hefesto_vivo.py:3485`): todo `RuntimeError` de gesto
...
def pacote(ctx: Contexto) -> dict[str, Any]:
    ...
    try:
        ...
        # ativo (`perfis_web.py:644`), então clicar numa linha mudava o alvo dos
...
def editor_nome(ctx: Contexto, o: dict[str, Any], p: Any) -> dict[str, Any] | None:
    ...
    (`hefesto_vivo.py:1579`) e o piloto escuta `change` além de `click`, que é o
