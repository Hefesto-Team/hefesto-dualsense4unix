...
série (`core/backend_pydualsense.py:5407`, `flag = bool(aceso)`, e `:453`,
...
def _mudo(backend: Any, uniq: str) -> bool | None:
    ...
    de INPUT (`core/backend_pydualsense.py:5484`). **Não é `microphone_mute_for`
...
def _baterias(backend: Any) -> dict[str, int]:
    ...
    (`core/backend_pydualsense.py:7391`) e a leitura é `getattr` no objeto que
...
def _escrever(backend: Any, uniq: str, valor: int | None) -> bool:
    ...
    **NÃO É `set_mic_led`.** Aquele esmaga em `bool` duas vezes em série e faz
    ...
    `core/backend_pydualsense.py:5407` e `:416`).
