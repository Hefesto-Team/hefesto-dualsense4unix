...
série (`core/backend_pydualsense.py:5408`, `flag = bool(aceso)`, e `:453`,
...
def _baterias(backend: Any) -> dict[str, int]:
    ...
    (`core/backend_pydualsense.py:7392`) e a leitura é `getattr` no objeto que
...
def _escrever(backend: Any, uniq: str, valor: int | None) -> bool:
    ...
    **NÃO É `set_mic_led`.** Aquele esmaga em `bool` duas vezes em série e faz
    ...
    `core/backend_pydualsense.py:5408` e `:416`).
