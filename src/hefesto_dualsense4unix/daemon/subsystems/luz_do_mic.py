...
série (`core/backend_pydualsense.py:5201`, `flag = bool(aceso)`, e `:453`,
...
(`core/backend_pydualsense.py:5435`), medido: `0->0, 1->1, 2->2, 3->3`.
...
def _mudo(backend: Any, uniq: str) -> bool | None:
    ...
    de INPUT (`core/backend_pydualsense.py:5278`). **Não é `microphone_mute_for`
...
def _baterias(backend: Any) -> dict[str, int]:
    ...
    (`core/backend_pydualsense.py:7122`) e a leitura é `getattr` no objeto que
...
async def _fora_do_laco(daemon: Any, fn: Any, *args: Any) -> Any:
    ...
    executor montado (`daemon/lifecycle.py:6028` afirma isso), e um daemon
...
def _escrever(backend: Any, uniq: str, valor: int | None) -> bool:
    ...
    **NÃO É `set_mic_led`.** Aquele esmaga em `bool` duas vezes em série e faz
    ...
    `core/backend_pydualsense.py:5201` e `:416`).
    ...
    **Controle que sai da mesa perde a memória inteira**, e não é higiene: na
    ...
    try:
        ...
        try:
            ...
            # (`daemon/connection.py:1824-1825`) — um laço cancelado não repinta e
