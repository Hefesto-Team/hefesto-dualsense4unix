...
série (`core/backend_pydualsense.py:4870`, `flag = bool(aceso)`, e `:425`,
...
def _mudo(backend: Any, uniq: str) -> bool | None:
    ...
    de INPUT (`core/backend_pydualsense.py:4947`). **Não é `microphone_mute_for`
...
async def _fora_do_laco(daemon: Any, fn: Any, *args: Any) -> Any:
    ...
    executor montado (`daemon/lifecycle.py:5164` afirma isso), e um daemon
...
def _escrever(backend: Any, uniq: str, valor: int | None) -> bool:
    ...
    **NÃO É `set_mic_led`.** Aquele esmaga em `bool` duas vezes em série e faz
    ...
    `core/backend_pydualsense.py:4870` e `:416`).
