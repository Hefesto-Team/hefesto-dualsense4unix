...
Por Bluetooth não há placa de áudio: nenhum controle no rádio entra aqui.
...
class Controle:
    """Um DualSense no CABO, com placa de som — o que o `winepulse` vê."""
    ...
    def prefixo_do_container(self) -> str:
        ...
        return f"{{{(self.pid << 16) | VID_SONY:08x}-{self.bus:04x}-{self.dev:04x}-"
...
def main(argv: list[str] | None = None) -> int:
    ...
    controles = [] if args.remover else controles_no_cabo(Path(args.sysfs), Path(args.udev_data))
    try:
